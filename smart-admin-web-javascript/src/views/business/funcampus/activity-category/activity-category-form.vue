...
<template>
  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" >
        ...
        <a-form-item label="活动类型名称"  name="name">
          <a-input style="width: 100%" v-model:value="form.name" placeholder="活动类型名称" />
        </a-form-item>
...
<script setup>
  ...
  const rules = {
      ...
      name: [{ required: true, message: '活动类型名称 必填' }],
  ...
  async function save() {
    ...
    try {
      if (form.id) {
        await activityCategoryApi.update(form);
      } else {
        await activityCategoryApi.add(form);
      }
