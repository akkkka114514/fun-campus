  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" >
        ...
        <a-form-item label="学院名称"  name="name">
          <a-input style="width: 100%" v-model:value="form.name" placeholder="学院名称" />
        </a-form-item>
        <a-form-item label="所属学校id"  name="schoolId">
          <a-input-number style="width: 100%" v-model:value="form.schoolId" placeholder="所属学校id" />
        </a-form-item>
...
<script setup>
  ...
  import { collegeInfoApi } from '/@/api/business/college-info/college-info-api';
  ...
  const rules = {
      ...
      name: [{ required: true, message: '学院名称 必填' }],
      schoolId: [{ required: true, message: '所属学校id 必填' }],
  ...
  async function save() {
    ...
    try {
      if (form.id) {
        await collegeInfoApi.update(form);
      } else {
        await collegeInfoApi.add(form);
      }
