...
export const noticeApi = {
  ...
  getAllNoticeTypeList() {
    return getRequest('/noticeType/getAll');
  },

  // 通知公告类型-添加 @author zhuoda
  addNoticeType(name) {
    return getRequest(`/noticeType/add/${name}`);
  },

  // 通知公告类型-修改 @author zhuoda
  updateNoticeType(noticeTypeId, name) {
    return getRequest(`/noticeType/update/${noticeTypeId}/${name}`);
  },
  // 通知公告类型-删除 @author zhuoda
  deleteNoticeType(noticeTypeId) {
    return getRequest(`/noticeType/delete/${noticeTypeId}`);
  },
  ...
  queryNotice(param) {
    return postRequest('/notice/query', param);
  },

  // 通知公告-添加 @author zhuoda
  addNotice(param) {
    return postRequest('/notice/add', param);
  },
  ...
  deleteNotice(noticeId) {
    return getRequest(`/notice/delete/${noticeId}`);
  },

  // 通知公告-更新详情 @author zhuoda
  getUpdateNoticeInfo(noticeId) {
    return getRequest(`/notice/getUpdateVO/${noticeId}`);
  },
