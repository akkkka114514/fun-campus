...
<template>
  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" >
        ...
        <a-form-item label="能报名的年级"  name="canEnrollGrade">
          <a-input-number style="width: 100%" v-model:value="form.canEnrollGrade" placeholder="能报名的年级" />
        </a-form-item>
...
<script setup>
  ...
  const formDefault = {
      ...
      canEnrollGrade: undefined, //能报名的年级
  ...
  const rules = {
      ...
      canEnrollGrade: [{ required: true, message: '能报名的年级 必填' }],
  ...
  async function save() {
    ...
    try {
      if (form.id) {
        await activityCanEnrollGradeApi.update(form);
      } else {
        await activityCanEnrollGradeApi.add(form);
      }
