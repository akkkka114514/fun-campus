  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" >
        ...
        <a-form-item label="年级"  name="name">
          <a-input style="width: 100%" v-model:value="form.name" placeholder="年级" />
        </a-form-item>
        ...
        <a-form-item label="修改时间"  name="udpateTime">
          <a-date-picker show-time valueFormat="YYYY-MM-DD HH:mm:ss" v-model:value="form.udpateTime" style="width: 100%" placeholder="修改时间" />
        </a-form-item>
    </a-form>
...
<script setup>
  ...
  import { gradeInfoApi } from '/@/api/business/grade-info/grade-info-api';
  ...
  async function save() {
    ...
    try {
      if (form.id) {
        await gradeInfoApi.update(form);
      } else {
        await gradeInfoApi.add(form);
      }
