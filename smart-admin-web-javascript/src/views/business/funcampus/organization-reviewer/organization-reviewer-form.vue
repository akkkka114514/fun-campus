  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" >
        ...
        <a-form-item label="组织id"  name="organizationId">
          <a-input-number style="width: 100%" v-model:value="form.organizationId" placeholder="组织id" />
        </a-form-item>
        <a-form-item label="后台用户id"  name="backendUserId">
          <a-input-number style="width: 100%" v-model:value="form.backendUserId" placeholder="后台用户id" />
        </a-form-item>
...
<script setup>
  ...
  import { organizationReviewerApi } from '/@/api/business/organization-reviewer/organization-reviewer-api';
  ...
  const formDefault = {
      ...
      backendUserId: undefined, //后台用户id
  ...
  const rules = {
      ...
      organizationId: [{ required: true, message: '组织id 必填' }],
      backendUserId: [{ required: true, message: '后台用户id 必填' }],
  ...
  async function save() {
    ...
    try {
      if (form.id) {
        await organizationReviewerApi.update(form);
      } else {
        await organizationReviewerApi.add(form);
      }
