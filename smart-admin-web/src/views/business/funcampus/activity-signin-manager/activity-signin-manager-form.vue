...
<template>
  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" >
        ...
        <a-form-item label="活动签到员主键"  name="portalUserId">
          <a-input-number style="width: 100%" v-model:value="form.portalUserId" placeholder="活动签到员主键" />
        </a-form-item>
        ...
        <a-form-item label="签到员用户名"  name="username">
          <a-input style="width: 100%" v-model:value="form.username" placeholder="签到员用户名" />
        </a-form-item>
    </a-form>
...
<script setup>
  ...
  const rules = {
      ...
      portalUserId: [{ required: true, message: '活动签到员主键 必填' }],
      ...
      username: [{ required: true, message: '签到员用户名 必填' }],
  };
  ...
  async function save() {
    ...
    try {
      if (form.id) {
        await activitySigninManagerApi.update(form);
      } else {
        await activitySigninManagerApi.add(form);
      }
