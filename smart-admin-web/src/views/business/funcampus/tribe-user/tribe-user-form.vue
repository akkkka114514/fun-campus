...
<template>
  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" >
        ...
        <a-form-item label="部落id"  name="tribeId">
          <a-input-number style="width: 100%" v-model:value="form.tribeId" placeholder="部落id" />
        </a-form-item>
        <a-form-item label="参与部落的前端用户id"  name="portalUserId">
          <a-input-number style="width: 100%" v-model:value="form.portalUserId" placeholder="参与部落的前端用户id" />
        </a-form-item>
...
<script setup>
  ...
  import { tribeUserApi } from '/@/api/business/tribe-user/tribe-user-api';
  ...
  const formDefault = {
      id: undefined, //主键
      tribeId: undefined, //部落id
      portalUserId: undefined, //参与部落的前端用户id
  ...
  const rules = {
      ...
      tribeId: [{ required: true, message: '部落id 必填' }],
      portalUserId: [{ required: true, message: '参与部落的前端用户id 必填' }],
  ...
  async function save() {
    ...
    try {
      if (form.id) {
        await tribeUserApi.update(form);
      } else {
        await tribeUserApi.add(form);
      }
