...
<template>
  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" >
        ...
        <a-form-item label="能报名的部落id"  name="canEnrollTribe">
          <a-input-number style="width: 100%" v-model:value="form.canEnrollTribe" placeholder="能报名的部落id" />
        </a-form-item>
...
<script setup>
  ...
  const formDefault = {
      ...
      canEnrollTribe: undefined, //能报名的部落id
  ...
  const rules = {
      ...
      canEnrollTribe: [{ required: true, message: '能报名的部落id 必填' }],
  ...
  async function save() {
    ...
    try {
      if (form.id) {
        await activityCanEnrollTribeApi.update(form);
      } else {
        await activityCanEnrollTribeApi.add(form);
      }
