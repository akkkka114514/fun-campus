  ...
  >
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" >
        ...
        <a-form-item label="学院id"  name="collegeId">
          <a-input-number style="width: 100%" v-model:value="form.collegeId" placeholder="学院id" />
        </a-form-item>
...
<script setup>
  ...
  import { collegeReviewerApi } from '/@/api/business/college-reviewer/college-reviewer-api';
  ...
  const rules = {
      ...
      collegeId: [{ required: true, message: '学院id 必填' }],
  ...
  async function save() {
    ...
    try {
      if (form.id) {
        await collegeReviewerApi.update(form);
      } else {
        await collegeReviewerApi.add(form);
      }
